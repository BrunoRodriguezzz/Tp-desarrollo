...
describe("test sobre la creacion de mensajes para las notificaciones con el factory de notificaciones", () =>{
    ...
    test("Mensaje cuando el estado del pedido no es uno válido del enum", ()=>{
        const estadoPedidoFalso = "no soy un estado de pedido, pero tengo la dignidad de reconocerlo"
        expect(()=>{
            factoryNotificacionEspaniol.crearSegunEstadoPedido(estadoPedidoFalso)
        }).toThrow(Error("El estado del pedido no corresponde con un objeto de su clase"))
    })
    test("Mensaje cuando el estado del pedido es uno válido", ()=>{
        const mensajeEspaniol = factoryNotificacionEspaniol.crearSegunEstadoPedido(estadoPendiente);
        const mensajeIngles = factoryNotificacionIngles.crearSegunEstadoPedido(estadoPendiente);
        expect(mensajeEspaniol).toBe("Nuevo pedido {id} realizado por {nombreComprador}.\nItems:\n{items}\nTotal: $ {total}\nDirección de entrega: {calle} {altura}, {piso} {departamento}. CP {codigoPostal}. {ciudad}, {provincia}, {pais}.")
        expect(mensajeIngles).toBe("New order {id} placed by {nombreComprador}.\nItems:\n{items}\nTotal: $ {total}\nDelivery address: {calle} {altura}, {piso} {departamento}. Zip {codigoPostal}. {ciudad}, {provincia}, {pais}.")
    })
    ...
    test("Cuando el pedido es correcto, el mensaje se forma exitosamente", ()=>{
      const mensajeBase = factoryNotificacionEspaniol.crearSegunEstadoPedido(estadoPendiente);
