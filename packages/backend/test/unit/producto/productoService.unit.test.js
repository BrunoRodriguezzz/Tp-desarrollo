...
import { ValidationError, NotFoundError } from '../../../errors/tiendaSolError.js';
...
describe('ProductoService', () => {
  ...
  test('update actualiza producto si existe', async () => {
    const prod = { id: 1, precio: 20, categorias: [] };
    mockRepo.findById.mockResolvedValue({ id: 1, categorias: [] });
  ...
  test('update lanza NotFoundError si no existe', async () => {
    ...
    await expect(productoService.update(999, { precio: 20 })).rejects.toThrow(NotFoundError);
  });
